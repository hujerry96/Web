---
title: 'Spectra Studio: A Free Spectral Plotting Tool for XRD, FTIR, RAMAN, UV-Vis & DSC'
slug: 'spectra-studio-guide-en'
description: 'A free spectral analysis and plotting tool for XRD, FTIR, RAMAN, UV-Vis and DSC/TGA data — multi-format loading, peak fitting, phase matching, journal-ready figure export, plus crystallite size (Scherrer/Williamson-Hall), crystallinity and Tauc bandgap analysis in Pro. Download and use the free edition now; Pro unlocks instantly with a key.'
keywords: ['XRD', 'FTIR', 'RAMAN', 'UV-Vis', 'DSC', 'spectral analysis', 'plotting tool', 'peak fitting', 'crystallite size', 'Scherrer', 'Williamson-Hall', 'crystallinity', 'Tauc bandgap', 'bandgap analysis', 'phase matching', 'journal figures', 'Spectra Studio']
author: 'Jerry Hu'
date: 2026-08-08
...
## 🎬 Meet Spectra Studio in 30 Seconds
...
## 🎬 60-Second Feature Tour: Every Tool, Live
...
- **Data loading**: instrument files read directly — PANalytical (.xrdml), Bruker (.brml), generic .uxd — plus .txt / .xy / .csv / .dat, FTIR (.asc), **UV-Vis (wavelength in nm, auto-detected)**, DSC/TGA (Excel .xlsx / .xls)
...
| Crystallite size: Scherrer crystallite size / Williamson-Hall strain / crystallinity | — | ✅ |
| Tauc bandgap analysis (UV-Vis, direct/indirect, thickness conversion) | — | ✅ |
| Phase matching (PDF cards) & Miller indices | — | ✅ |
| Derivative / integral calculus | — | ✅ |
| Vector export (PDF / SVG / EPS / TIFF) & high DPI | — | ✅ |

Pro is a natural fit when:

- **Submitting papers & reports**: PDF / SVG / EPS vector export + journal styles — figures go into the manuscript as-is, crisp at any zoom
- **Labs & teaching**: phase matching auto-labels Miller indices, crystallite size in one click — a much shorter learning curve for students and colleagues
- **Analytical centers & industry**: multi-sample management + Tauc bandgap + Williamson-Hall in one routine workflow

**Pro is already complete**: every Pro feature is finished and built into the v2.1.1 installer — nothing extra to download. After purchase, paste the key you receive by email under "🔑 Unlock Pro" in the app (one online verification on first activation, offline afterwards).

> ### 🛒 Unlock every Pro feature now
>
...
> - **30-day money-back guarantee** — if it is not for you, zero risk
> - Key (`SS_-XXXX-...`, up to 2 devices) arrives instantly in your receipt email; re-paste it after reinstalling or switching machines
...
1. **Setup program** (recommended): [Download SpectraStudio-Setup-v2.1.1.exe (120 MB)](https://github.com/hujerry96/Web/releases/latest/download/SpectraStudio-Setup-v2.1.1.exe), double-click to install into the current user's directory (no administrator rights needed), with Start-menu shortcut and uninstaller.
2. **Portable build**: [Download SpectraStudio-v2.1.1-portable.exe (120 MB)](https://github.com/hujerry96/Web/releases/latest/download/SpectraStudio-v2.1.1-portable.exe) and double-click to run.
...
- Setup `SpectraStudio-Setup-v2.1.1.exe`: `6484aae6f2ce08e27ecb6caaf9485dfc37e6bb0047925882be8b37e178795579`
- Portable `SpectraStudio-v2.1.1-portable.exe`: `166a13b4f3eceb038a96235e95e6bd4a759b3789309e276dda749fdd80fef40b`
...
**Q: How are crystallite size and strain computed?**
Click "Crystallite Size" (since v2.1.1 no peak fitting is needed first — the dialog can auto-detect peaks; you may still fit peaks first to get each FWHM) — the Scherrer equation (D=Kλ/βcosθ) computes the crystallite size of every peak automatically; with 2+ peaks a Williamson-Hall plot gives microstrain ε and R² as well. Wavelength and shape factor K are adjustable.

**Q: Can I do bandgap analysis on UV-Vis data?**
Yes. v2.1 auto-detects UV-Vis data (wavelength in nm); click "Tauc Bandgap" — switch between direct/indirect bandgaps in one click, the linear region is detected automatically, and Eg is annotated on the plot. You can enter film thickness for the absorption coefficient, or fine-tune the linear region manually by clicking twice on the plot.
