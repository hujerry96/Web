...
## Deep Analysis (Pro): Crystallite Size, Crystallinity & Tauc Bandgap
...
Run peak fitting on the sample (Step 4), then click "Deep Analysis":
...
After fitting, the crystallinity tab of Deep Analysis computes **crystallinity = fitted peak area / total area in the fit range**, with the integration range drawn on the plot. Works directly for samples with a flat background; for large amorphous humps (e.g. polymers), apply baseline correction first.
...
- **Deep analysis (Pro)**: Scherrer crystallite size, Williamson-Hall strain, crystallinity, Tauc bandgap (see section above)
...
| Deep analysis: Scherrer crystallite size / Williamson-Hall strain / crystallinity | — | ✅ |
...
**Pro is already complete**: every Pro feature is finished and built into the v2.1.0 installer — nothing extra to download. After purchase, paste the key you receive by email under "🔑 Unlock Pro" in the app (one online verification on first activation, offline afterwards).
...
1. **Setup program** (recommended): [Download SpectraStudio-Setup-v2.1.0.exe (120 MB)](https://github.com/hujerry96/Web/releases/latest/download/SpectraStudio-Setup-v2.1.0.exe), double-click to install into the current user's directory (no administrator rights needed), with Start-menu shortcut and uninstaller.
2. **Portable build**: [Download SpectraStudio-v2.1.0-portable.exe (120 MB)](https://github.com/hujerry96/Web/releases/latest/download/SpectraStudio-v2.1.0-portable.exe) and double-click to run.
...
- Setup `SpectraStudio-Setup-v2.1.0.exe`: `13f5310bbae4f613af4dcc408c867f5b93c6c5c17c8ac71131f8d1a14ea47a1d`
- Portable `SpectraStudio-v2.1.0-portable.exe`: `1dffcfa3f24791b74e8790cf0e9be368aeb48528d545b7146811fbb21e3a5ae7`
...
Run peak fitting to get each peak's FWHM, then click "Deep Analysis" — the Scherrer equation (D=Kλ/βcosθ) computes the crystallite size of every peak automatically; with 2+ peaks a Williamson-Hall plot gives microstrain ε and R² as well. Wavelength and shape factor K are adjustable.
