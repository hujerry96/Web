...
- **MoS₂ (molybdenum disulfide)**: The most-studied n-type 2D semiconductor, with a bandgap of ~1.8 eV (monolayer) and mobility exceeding 200 cm²/V·s under ideal conditions.
- **WS₂ (tungsten disulfide)**: Structurally similar to MoS₂ but with stronger optical properties, making it suitable for optoelectronics.
- **WSe₂ (tungsten diselenide)**: One of the few p-type TMDs — a critical material for achieving complementary logic (CMOS).
- **Black phosphorus**: Tunable bandgap (0.3–2 eV) and high mobility, but oxidizes at a terrifying rate.
- **hBN (hexagonal boron nitride)**: Technically an insulator, but indispensable as the "ideal substrate" and dielectric layer for 2D materials.

They share a common trait: **all are layered materials held together by van der Waals forces, which can be peeled apart one layer at a time using mechanical exfoliation.** That is what makes them so appealing — perfect crystals without needing to be grown; just peel them from a bulk crystal.

But what you peel off is only micrometers in size. For mass production, you need to "grow" these films on large-area substrates using chemical vapor deposition (CVD) and similar methods. And that is where the first bottleneck begins.

## Bottleneck 1: Contact Resistance — Metal Just Does not Work Well
...
## Bottleneck 2: Large-Area Growth — Lab Flakes ≠ Wafer-Scale Films
...
Black phosphorus is far worse — it begins oxidizing within minutes of air exposure, with phosphoric acid droplets forming on the surface. This makes practical applications extremely difficult unless the entire process is carried out in an inert atmosphere, or the material is fully encapsulated with hBN.

**Long-term reliability** is also a hidden concern for industrialization. Silicon devices have decades of accumulated reliability data, and failure rate curves are thoroughly understood. The longest reliability data for 2D semiconductor devices spans only a few years — a massive gap compared to the 10–20 year lifetime validation required for automotive or industrial-grade applications.

## Bottleneck 4: Doping Control — No Doping Means No CMOS
...
Current alternatives fall into three categories. One is **substitutional doping**, where Nb (niobium) or V (vanadium) atoms are mixed directly into the CVD precursors, replacing Mo atoms to achieve p-type doping. Recent 2025 progress shows Nb-doped WSe₂ achieving hole mobility of 48 cm²/V·s with an on/off ratio of 10⁸. But concentration control remains coarse, and doping uniformity over large areas is still poor.
...
## Bottleneck 5: Silicon Process Integration — How to Play Nice with Existing Fabs
...
This imposes several hard constraints. First, **growth temperature must stay below 400°C** — because 2D semiconductors are typically integrated at the back-end-of-line (BEOL), after metal interconnect layers are already in place. Temperatures above 400°C damage the underlying copper wiring and low-κ dielectrics. Current CVD growth of MoS₂ typically requires 700–1000°C, far exceeding this limit. Low-temperature CVD and atomic layer deposition (ALD) methods are under development, but quality still lags.
...
Intel, TSMC, and Samsung have all invested in 2D semiconductor process integration research, but all remain at the proof-of-concept stage — at least 5–10 years away from volume manufacturing integration.
